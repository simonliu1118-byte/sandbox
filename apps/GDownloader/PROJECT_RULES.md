# GDownloader Project Rules

本檔只保存 GDownloader 的固定專案規則；repository 共通版本、開發、CI、Local-first 與 GitHub-final verification 規則依根 `REPOSITORY_RULES.md`。

## 1. 產品、平台與版本線

- 正式技術線：C# / .NET 8 / WinForms + WebView2；目標平台 Windows x64。
- 正式發行以 self-contained portable folder 為目標，不要求使用者另外安裝 .NET runtime。
- `GDownloader.exe` 是本專案唯一產品識別，不建立第二個平行專案或另一條版號。
...
- WebView2 使用固定的持久化 profile 目錄（本機 `%LOCALAPPDATA%\GDownloader\WebView2Profile`），登入狀態預期可跨程式重啟保留，使用者不需每次重新登入。
...
- `yt-dlp.exe` 與 `ffmpeg.exe` 屬執行期依賴，於程式第一次需要時自動下載至本機 `%LOCALAPPDATA%\GDownloader\tools`，不隨 Git 提交，也不內嵌於本專案 build 產物中。
...
- `cookies.txt`、WebView2 profile 目錄、下載佇列狀態與任何登入相關資料屬使用者 runtime 個資，一律保存於本機 `%LOCALAPPDATA%\GDownloader`，不得提交至 Git、記錄於 log 範例或作為測試 fixture。
- 下載完成的影片檔案預設輸出至使用者本機資料夾（預設 `Downloads\GDownloader`），不隨程式或 Git 一併保存。
