...
- 新功能預設延伸既有共用流程、service 與資料模型，避免平行流程、局部特判、第二套狀態與重複實作造成疊床架屋。
- 例外流程允許存在，但必須有明確業務、資料覆蓋、安全或技術理由，且維持最小範圍；不得再無限制衍生第二層、第三層例外。
- AI 建議新增例外時，必須先說明一般流程不足、例外目的、影響範圍、維護／測試成本與可替代方案，由使用者決定是否採用。
- 使用者提出的需求若會形成新的資料語意、狀態、UI 或流程特判，AI 也必須先提醒會形成例外，再依使用者決定執行。
- 已核准例外依既有三層治理落點管理：永久專案例外進唯一 `PROJECT_RULES.md`，一次性例外留在 PR／Issue／設計說明，不新增平行規則檔。
